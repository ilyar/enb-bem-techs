...
module.exports = {
    ...
    toBemdecl: function (deps) {
        ...
        return deps.map(function (dep) {
            ...
            dep.elem ?
                (item.elems = (Array.isArray(dep.elem) ? dep.elem : [dep.elem])
                    .map(function (name) {
                        return applyModVal(dep, { name: name });
                    }))
                : applyModVal(dep, item);

            return item;
        });

        function applyModVal(dep, item) {
            dep.mod && (item.mods = [{ name: dep.mod }],
                dep.val && (item.mods[0].vals = [{ name: dep.val }]));
            return item;
        }
    },
